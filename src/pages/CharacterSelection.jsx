    ...
    <div className="min-h-screen w-full bg-dungeon-black safe-area-top safe-area-left safe-area-right container-mobile-safe">
              ...
              {characterClasses.map((charClass) => {
                ...
                return (
                  <Card
                    ...
                    className={`
                      bg-stone-charcoal/80 border-2 text-white text-center p-4 sm:p-6
                      ...
                      ${!isClickable ? "opacity-40 cursor-not-allowed" : "cursor-pointer hover:border-frost-blue active:scale-95"}
                    `}
                  >
                    <div className="text-4xl sm:text-6xl mb-3 sm:mb-4">{charClass.icon}</div>
                    <h3 className={`text-xl sm:text-2xl font-bold ${charClass.color} mb-2`}>
                      {charClass.name}
                    </h3>
                    <p className="text-xs sm:text-sm text-stone-light mt-2 h-8 sm:h-10 line-clamp-2">
                      {charClass.description}
                    </p>
                    {isSelectedByMe && (
                      <p className="text-crystal-blue font-bold mt-2 text-sm sm:text-base">
                        Você é {mySelection.name}
                      </p>
                    )}
                    {isTakenByOther && (
                      <p className="text-red-400 font-bold mt-2 text-xs sm:text-sm">
                        Já escolhido
                      </p>
                    )}
                  </Card>
            ...
            {isHost && (
              <div className="text-center space-y-3">
                <Button
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white font-bold text-base sm:text-xl py-4 sm:py-6 px-6 sm:px-10 w-full disabled:opacity-50 disabled:cursor-not-allowed min-h-[52px] sm:min-h-[60px]"
                >
                  Iniciar Jogo
                </Button>
                {!allPlayersReady && (
      ...
      >
        <DialogContent className="bg-stone-charcoal border-stone-light/30 text-white max-w-[90vw] sm:max-w-md mx-4">
          <DialogHeader>
            <DialogTitle className="text-xl sm:text-2xl text-ethereal-blue">
              Escolha seu {selectedClass?.name}
            </DialogTitle>
            <DialogDescription className="text-stone-light text-sm">
          ...
          <div className="flex flex-col space-y-2 sm:space-y-3 pt-4 max-h-[60vh] overflow-y-auto">
            {selectedClass?.heroes.map((hero) => (
              <Button
                ...
                variant="outline"
                size="lg"
                className="bg-ancient-stone hover:bg-weathered-gray border-stone-light/30 text-base sm:text-lg font-semibold min-h-[48px] w-full"
        ...
        <div className="flex flex-col gap-2 max-w-sm mx-auto">
          {isHost && (
            <Button
              onClick={startGame}
              disabled={!allPlayersReady}
              className="w-full bg-green-600 hover:bg-green-700 text-white font-bold text-sm min-h-[48px] disabled:opacity-50"
            >
          ...
          <Button 
            onClick={handleLeaveRoom} 
            variant="outline"
            className="w-full bg-transparent border-stone-light/30 text-stone-light hover:text-white hover:bg-stone-light/10 font-bold text-sm min-h-[48px]"
          >
            Sair da Sala
          </Button>
        </div>
