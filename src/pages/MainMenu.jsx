          ...
          <Card className="w-full max-w-sm sm:max-w-md mx-auto bg-stone-charcoal/80 border-stone-light/20 text-white">
            ...
            <CardContent className="p-4 sm:p-6 pt-0">
              <div className="flex flex-col space-y-3 sm:space-y-4">
                <Button onClick={createRoom} variant="outline" size="lg" className="bg-arcane-blue hover:bg-crystal-blue text-white font-bold text-base sm:text-lg border-frost-blue/50 hover:text-white w-full min-h-[48px] sm:min-h-[52px] transition-all duration-200">
                  <Swords />
                  <span>Criar Nova Sala</span>
                </Button>
                <Button onClick={goToJoinRoom} variant="secondary" size="lg" className="bg-weathered-gray hover:bg-stone-light text-white font-bold text-base sm:text-lg border-stone-light/50 w-full min-h-[48px] sm:min-h-[52px] transition-all duration-200">
                  <DoorOpen />
                  <span>Entrar em uma Sala</span>
                </Button>
                <Button onClick={goToProfile} variant="outline" size="lg" className="bg-void-purple/80 hover:bg-void-purple text-white hover:text-white font-bold text-base sm:text-lg border-stone-light/50 w-full min-h-[48px] sm:min-h-[52px] transition-all duration-200">
                  <ScrollText />
                  <span className="hidden sm:inline">Ver Perfil e Histórico</span>
                  <span className="sm:hidden">Perfil e Histórico</span>
                </Button>
                <Button onClick={handleLogout} variant="destructive" size="lg" className="font-bold text-base sm:text-lg w-full min-h-[48px] sm:min-h-[52px] mt-4 sm:mt-6 transition-all duration-200">
                  <LogOut />
                  <span>Sair</span>
                </Button>
              </div>
